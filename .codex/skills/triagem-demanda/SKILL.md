...
description: Valida a classificacao da demanda, trata pedidos claramente investigativos como analise e aplica o protocolo correto para analise, bug ou implementacao.
---
...
- `implementacao` cobre melhoria, evolucao e nova funcionalidade; subtipo pode ser usado apenas para rastreabilidade.
...
- Em `bug` e `implementacao`, o agente deve seguir para plano obrigatorio antes de implementar.
...
3. Se nao for claramente analitica, verifique se o usuario informou um dos tipos de implementacao aceitos (`bug` ou `implementacao`).
...
Ao concluir a triagem, responda sempre com:

- `tipo`: `analise`, `bug` ou `implementacao`
- `escopo`: resumo curto do problema/objetivo
- `criterios_aceite`: lista objetiva do que define entrega correta
- `bloqueios`: lista de pendencias para seguir (ou `nenhum`)
